...
class SyncJobRepository:
    ...
    def get_by_connection(
        self, connection: SyncConnection, limit: int = 50
    ) -> List[SyncJob]:
        """Get jobs for a connection."""
        return list(
            SyncJob.objects.filter(connection=connection).order_by("-started_at")[
                :limit
            ]
        )
    ...
    def create_job(
        self, connection: SyncConnection, is_full_sync: bool = False
    ) -> SyncJob:
        """Create a new sync job."""
        job = SyncJob.objects.create(
            connection=connection, is_full_sync=is_full_sync, status="running"
        )
        return job
    ...
    def get_recent_jobs(self, limit: int = 100) -> List[SyncJob]:
        ...
        return list(
            SyncJob.objects.select_related("connection", "connection__user").order_by(
                "-started_at"
            )[:limit]
        )
