    ...
    def handle(self, *args, **options):
        ...
        for user in users:
            ...
            for category in categories:
                ...
                existing_count = CategoryKeyword.objects.filter(user=user, category=category).count()

                if existing_count > 0 and not force:
                    self.stdout.write(f"  Skipping {category.name} - already has {existing_count} keywords")
                    continue
                ...
                if force and existing_count > 0:
                    CategoryKeyword.objects.filter(user=user, category=category).delete()
                    self.stdout.write(f"  Deleted {existing_count} existing keywords for {category.name}")
                ...
                self.stdout.write(self.style.SUCCESS(f"  Added {len(keywords_to_add)} keywords to {category.name}"))
            ...
            if user_keywords_created > 0:
                ...
                self.stdout.write(f"No keywords needed for user {user.username} (ID: {user.id})")
