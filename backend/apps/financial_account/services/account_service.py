...
class AccountService:
    ...
    def get_household_accounts(
        self, user_ids: list[int], active_only: bool = True,
    ) -> list[FinancialAccount]:
        """Get shared accounts for a set of household member user IDs."""
        return self.account_repository.get_by_user_ids_shared(
            user_ids, is_active=active_only if active_only else None,
        )

    def get_account_by_id(
        self, account_id: int, user: User, household_user_ids: list[int] | None = None,
        ...
        if (
            household_user_ids
            and account.user_id in household_user_ids
            and account.shared_with_household
        ):
            return account
        return None
