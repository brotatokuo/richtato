"""Per-user envelope encryption for bank sync secrets.

Only two payloads in this app are ever encrypted: Playwright ``storage_state``
JSON blobs and per-account activity URLs (which embed bank-specific tokens
like BofA's ``adx``). The bank password is never persisted in any form.
