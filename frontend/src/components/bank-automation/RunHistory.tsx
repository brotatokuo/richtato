...
const STATUS_ICON: Record<BankAutomationRun['status'], typeof CheckCircle2> = {
  running: Loader2,
...
const STATUS_COLOR: Record<BankAutomationRun['status'], string> = {
...
export function RunHistory({ connectionId }: RunHistoryProps) {
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [runs, setRuns] = useState<BankAutomationRun[] | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError(null);
      try {
        const list = await bankAutomationApi.listRuns(connectionId);
