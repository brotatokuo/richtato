def sort_statements_and_generate_csv(request) -> HttpResponse:
    ...
    bank_list = [
        folder
        for folder in os.listdir(statements_folder_path)
        if os.path.isdir(os.path.join(statements_folder_path, folder))
    ]
    ...
    for bank_name in bank_list:
        ...
        if "amex" in bank_name.lower() or "american express" in bank_name.lower():
            ...
            print(
                colorama.Fore.RED
                + f"Bank {bank_name} not supported"
        ...
        output_filename = (
            f"viz/static/historic_data/alan_{datetime.now().strftime('%Y%m%d')}.csv"
        )
...
def handle_amex_statements(folder_path, bank):
    ...
    print(
        ...
        + f"Renamed {count} statements for {bank}"
        + colorama.Style.RESET_ALL
    )
...
def handle_bofa_statements(folder_path, bank):
    ...
    for statement in statements_list:
        ...
        rename_statements(
            excel_path, folder_path, bank, "Custom Cash", min_date, max_date
        )
...
def rename_statements(
    excel_path, folder_path, bank, account, min_date, max_date
) -> None:
...
def import_statements(request):
    ...
    account_list = list(
        CardAccount.objects.filter(user=request.user).values_list("name", flat=True)
    )
    ...
    try:
        ...
        for index, row in df_combined.iterrows():
            ...
            if Expense.objects.filter(
                ...
                print(
                    f"Skipping duplicate: {description} | {amount} | {date} | {account}"
                )
                continue

            print(
                f"Processing: {description} | Amount: {amount} | Date: {date} | Account: {account}"
            )
            ...
            if account not in account_list:
                print(
                    colorama.Fore.RED
                    + "Users available accounts: "
                    + str(account_list)
                    + colorama.Style.RESET_ALL
                )
                return HttpResponse(
                    f"Account {account} not found in available accounts. Please add the account first."
                )
            ...
            if category_instance == None:
                ...
                category_name = AI.categorize_transaction(
                    user=request.user, input=description
                )
                print(
                    f"Category found by AI: {category_name} | Call counter: {call_counter}"
                )
                category_instance = categories.get(name=category_name)

            # Create and save the transaction
            try:
                account_instance = CardAccount.objects.get(
                    user=request.user, name=account
                )
                save_transaction(
                    request.user,
                    account_instance,
        ...
        return HttpResponse(f"Error: {str(e)}")
    return HttpResponse(
        f"Processed {transactions_processed} out of {total} transactions."
    )
