...
def import_statements(request):
    ...
    account_list = list(CardAccount.objects.filter(user=request.user).values_list('name', flat=True))
    ...
    try:
        ...
        for index, row in df_combined.iterrows():
            ...
            if category_instance == None:
                ...
                category_name = AI.categorize_transaction(user=request.user, input=description)
            ...
            try:
                account_instance = CardAccount.objects.get(user=request.user, name=account)
                save_transaction(request.user, account_instance, description, category_instance, date, amount)
