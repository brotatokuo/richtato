...
def _get_line_graph_data(user: User, model, months: int | None = None) -> dict:
    # Set the timezone to US/Pacific
    pst = pytz.timezone("US/Pacific")
    today = datetime.now(pst).date()
    
    if months is not None:
        # If months is provided, calculate start date based on months back
        months = int(months)
        start_date = today - relativedelta(months=months)
        start_date = start_date.replace(day=1)
    else:
        # If months is None, get the earliest date from the data
