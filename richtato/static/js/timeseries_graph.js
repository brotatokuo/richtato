class TimeseriesGraph {
     ...
     */
    async initialize(defaultRange = '3') {
        this.selectedRange = defaultRange;
        await this.updateChart();

        // Set up dropdown event listener if it exists
        const monthsDropdown = document.getElementById("monthsDropdown");
        if (monthsDropdown) {
            monthsDropdown.value = defaultRange;
            monthsDropdown.addEventListener('change', async () => {
                this.selectedRange = monthsDropdown.value;
                await this.updateChart();
            });
        }
    }
     ...
     */
    async fetchData() {
        const endpointUrl = `${this.endpointBaseUrl}?month_range=${encodeURIComponent(this.selectedRange)}`;
        console.log("Fetching data with month range:", this.selectedRange);
